quick_arg_check <- function(args) {
  if ("output" %in% names(args) & !"output_file" %in% names(args)) {
    warning(paste0(
      "output passed, but not output_file, ",
...
#'     warning(
#'       paste0(
#'         "soffice does not seem configured properly, may need to ",
...
#'     url <- paste0(
#'       "https://codeyarns.github.io/tech/2019-09-05",
...
html_to_ari <- function(path,
  ...
  if (!requireNamespace("pagedown", quietly = TRUE)) {
    stop(
      paste0(
        "pagedown package needed to use chrome_print",
        " for html_to_ari"
      )
    )
  }
...
#' @rdname gs_to_ari
#' @export
pdf_to_pngs <- function(path, verbose = TRUE,
                        dpi = 600) {
  fmts <- pdftools::poppler_config()$supported_image_formats
  if ("png" %in% fmts) {
    format <- "png"
